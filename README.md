...
本项目收集 Stata 官方 `base` 目录中的 `.sthlp` 帮助文档，基于**本地部署的 GPT 模型**进行批量翻译。模型经过金融统计领域语料的 LoRA 微调，显著提升了术语准确性与风格一致性。同时，项目设计了 SMCL 标记语言的“隔离-保护-恢复”机制，确保翻译后的帮助文档格式零丢失、在 Stata 中正确渲染，与原生帮助体验完全一致。翻译完成后，所有文档被打包为 Stata 扩展命令 **`xhelp`**，只需运行一条安装脚本即可完成部署。
